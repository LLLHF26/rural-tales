import {EncodingType} from '../types/index';
...
export class ZipProvider extends BaseProvider {
  ...
  addFile(
    path: string,
    content: any,
    encoding: EncodingType = EncodingType.ENC_UTF8,
  ) {
    this.files.push({
      content,
      encoding,
   ...
   */
  async serveFiles({
    type = EncodingType.ENC_UTF8,
    compress = 0,
  }: {
    type: EncodingType;
    compress: number;
  }): Promise<
    string | Array<number> | Uint8Array | ArrayBuffer | Blob | Buffer
  > {
