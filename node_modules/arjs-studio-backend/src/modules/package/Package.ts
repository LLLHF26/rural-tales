  ...
  async serve(config) {
    ...
    switch (this.arType) {
      case AR_BARCODE:
        ...
        generatedHtml = MarkerModule.generateBarcodeHtml(
      ...
      case AR_PATTERN:
        generatedHtml = MarkerModule.generatePatternHtml(
          this.assetType,
          this.assetParam,
          `assets/${this.assetName}`,
        );
      ...
      case AR_LOCATION:
        generatedHtml = LocationModule.generateHtml(
          this.assetType,
          this.assetParam,
          `assets/${this.assetName}`,
        );
    ...
    switch (this.assetType) {
      ...
      case ASSET_VIDEO:
        if (provider instanceof ZipProvider) {
          ...
          provider.addFile(
            `assets/${this.assetName}`,
            arrayBufferToBase64(this.assetFile),
            ENC_BASE64,
          );
        }
        break;
